...
| Repudiation | Usuário nega ter feito determinada aplicação de insumo | Registros de auditoria com timestamp e usuário responsável em `AplicacaoInsumo` (campo `created_by`, `created_at` imutáveis enquanto o registro existe). **Mitigação parcial** (ADR 009): o próprio dono da conta pode apagar a trilha excluindo o `Plantio` pai (`AplicacaoInsumo.plantio` é `CASCADE`, não `PROTECT`) — decisão consciente de priorizar controle do usuário sobre os próprios dados nesse app de uso pessoal; a garantia vale contra terceiros, não contra o próprio dono da conta |
